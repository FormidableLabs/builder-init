...
Task.prototype.init = function (callback) {
  ...
  var archName = path.basename(archetype);
  ...
  async.auto({
    ...
    npmPack: ["tmpDir", function (cb, results) {
      ...
      var proc = childProc.spawn("npm", ["pack", archetype], {
        cwd: results.tmpDir,
        env: self.env,
        stdio: "inherit"
      });
      ...
      proc.on("close", function (code) {
        cb(code === 0 ? null : new Error(
          "'npm pack " + archetype + "' exited with error code: " + code));
      });
    }],
    ...
    init: ["extracted", function (cb, results) {
      ...
      try {
        init = require(initPath); // eslint-disable-line global-require
    ...
    checkTemplates: ["extracted", function (cb, results) {
      ...
      fs.stat(templatesPath, function (err, stats) {
        if (err) {
          if (err.code === "ENOENT") {
            return cb(new Error("Archetype '" + archName + "/init' directory not found"));
          }
        ...
        if (!stats.isDirectory()) {
          return cb(new Error("Archetype '" + archName + "/init' exists, but is not a directory"));
        }
