 ...
 * @param {Object} opts.version   Script version. (Default: `UNKNOWN`)
 * @param {Object} opts.initFile  Initialization file.
 * @param {Object} opts.initDir   Initialization / templates directory.
 * @returns {void}
 */
  ...
  async.auto({
    ...
    data: ["extracted", "dataExists", function (results, cb) {
      ...
      try {
        ...
        return void cb(null, _.extend(data, overrides));
    ...
    checkTemplates: ["extracted", function (results, cb) {
      var initDir = self.initDir; // TODO: ABSTRACT / TAKE FROM PROMPTS
      var templatesPath = path.join(results.tmpDir, "extracted", initDir);
      fs.stat(templatesPath, function (err, stats) {
        if (err) {
          if (err.code === "ENOENT") {
            return void cb(
              new Error("Module '" + self.modName + "/" + initDir + "' directory not found"));
          }
        ...
        if (!stats.isDirectory()) {
          return void cb(new Error(
            "Module '" + self.modName + "/" + initDir + "' exists, but is not a directory"));
        }
    ...
    var templatesPath = results.checkTemplates;
