...
Task.prototype.inflate = function (callback) {
  ...
  async.auto({
    ...
    dataExists: ["extracted", function (results, cb) {
      var dataPath = path.resolve(results.tmpDir, "extracted/init.js"); // TODO: ABSTRACT INIT
    ...
    data: ["extracted", "dataExists", function (results, cb) {
      ...
      try {
        ...
        // TODO: ABSTRACT INIT
        err.message += "\n[" + script + "] Error while importing '" + self.modName + "/init.js'";
    ...
    checkTemplates: ["extracted", function (results, cb) {
      var templatesPath = path.resolve(results.tmpDir, "extracted/init"); // TODO: ABSTRACT INIT
      fs.stat(templatesPath, function (err, stats) {
        if (err) {
          if (err.code === "ENOENT") {
            // TODO: ABSTRACT INIT
            return void cb(new Error("Module '" + self.modName + "/init' directory not found"));
          }
        ...
        if (!stats.isDirectory()) {
          return void cb(new Error(
            // TODO: ABSTRACT INIT
            "Module '" + self.modName + "/init' exists, but is not a directory"));
        }
    ...
    callback(null, {
      ...
      // TODO: ABSTRACT END
      src: path.resolve(results.tmpDir, "extracted/init") // TODO: ABSTRACT INIT
    });
  });
};
